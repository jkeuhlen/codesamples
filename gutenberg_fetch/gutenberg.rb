...
# External Gems: gutenberg_rdf : https://github.com/mrcook/gutenberg_rdf
...
# Call the setup method if your base_dir has not been set up already. This is a naive check and will not look for contents of that directory, only its existence.
...
unless (File.file?('titles.yml'))
  ...
  Dir.glob(@base_dir + '*/*.rdf') do |rdf|
    ...
    #TODO Audiobooks could be combined but they are typically stored under a different path and in a different format so combining them becomes non-trivial
    unless (book.type.eql? 'Text')
      next
    end
